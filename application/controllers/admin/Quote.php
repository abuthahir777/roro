...
class Quote extends CI_Controller 
{
	public function __construct()
	{
		...
		$this->load->model(array('Quotation_Model','Country_Model','Airport_Model'));

		$this->page = $this->config->item("base_url_admin")."/quote";
		...
		$this->permission = $this->permission->setRights($this->session->userdata('roleId'),15);
	
		$this->permission = array('create'=>1, 'update'=>1, 'delete'=>1, 'status'=>1, 'view'=>1);
	}
	...
	function index()
	{
		...
		$this->layouts->title('Quotaion Table');
		$this->layouts->view('pages/admin/quotation/table',$data,'admin');
	}

	function fetch()
	{
		$fetch_data = $this->Quotation_Model->fetch_data();
